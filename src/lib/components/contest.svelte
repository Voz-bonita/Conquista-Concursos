...
<div class="page-body">
	<QuestionNavigationRadio {questions} bind:currentQuestionIndex={questionIndex} />
	{#if doingContest}
		<ObjectiveQuestion
			question={questions[questionIndex]}
			{questionIndex}
			bind:userSelected={answers[questionIndex]}
		/>
		<div class="directional-btns-div">
			{#if questionIndex == 0}
				<button
					class="navigattion-btn cancel-btn clickable"
					on:click|preventDefault={cancel_contest}>Cancelar</button
				>
			{:else}
				<button
					class="navigattion-btn back-btn clickable"
					on:click|preventDefault={previous_question}>Anterior</button
				>
			{/if}
			{#if questionIndex == questions.length - 1}
				<button class="navigattion-btn end-btn clickable" on:click|preventDefault={submit_results}
					>Finalizar</button
				>
			{:else}
				<button class="navigattion-btn next-btn clickable" on:click|preventDefault={next_question}
					>Próxima</button
				>
			{/if}
		</div>
	{:else}
		...
		<ObjectiveQuestionAnswers {questions} index={questionIndex} {answers} />
	{/if}
</div>
